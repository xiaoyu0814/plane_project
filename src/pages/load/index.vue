...
<script>
import {getUserMsg} from '../../api/load.js'
export default {
  name: 'Load',
  data () {
  ...
  methods: {
    //  登录方法没有用vuex触发
    handleSubmit (name) {
		 //window.localStorage.setItem("userId",'12345678');
		 this.$router.push("/");
		 window.localStorage.setItem("userId",'12345678');
		getUserMsg(this.formInline.user,this.formInline.password).then((data)=>{
			if(data.returnCode=="100"){
				//window.localStorage.setItem("userId",'12345678');
				this.$Message.info("登录成功");
				this.$router.push("/");
			}else{
				this.$Message.info("账号/密码错误");
			}
		})
