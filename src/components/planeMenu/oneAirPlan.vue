<template>
  ...
  >
    ...
    <Button long @click="oneQuery">查询</Button>
...
export default {
  ...
  methods: {
    openByDrop(e) {
      ...
      this.calendar3.left = e.target.offsetLeft + 92;
      this.calendar3.top = e.target.offsetTop + 141;
    ...
    show() {
      this.oneFlyMonitor_show = true;
    },
    close() {
      this.map.remove("onePlaneLine");
      this.map.remove("onePlanePoint");
      this.oneFlyMonitor_show = false;
    ...
    oneQuery() {
      ...
      this.$api.get(url, params).then((res) => {
        ...
        //_this.map.addAirrouteCongestion(res, "airrouteCongestion");
      });
    },
    setStartTime(e) {
      this.flightDate = e;
