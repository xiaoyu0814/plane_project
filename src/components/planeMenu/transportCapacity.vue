<template>
  <!-- 运力综合统计 -->
  <Modal title="运力综合统计" v-model="transportCapacity_show" footer-hide fullscreen @on-cancel="close">
    <Tabs type="card" @on-click="transportCapacity_type">
      <TabPane label="按省份统计">
        <table class="transportCapacity_table">
          <tr>
            <td>
              <span class="nowtime">当前时刻</span>
            </td>
            <td>
              <DatePicker
                v-if="false"
                v-model="transportCapacity_city_time"
                type="datetime"
                placeholder="请选择时间"
                style="width: 200px"
              ></DatePicker>
              <input
                class="DatePicker"
                type="text"
                @click="openByDrop($event)"
                v-model="calendar3.display"
                readonly
              />
            </td>
            <td>
              <Select v-model="transportCapacity_city" style="width:200px">
                <Option
                  v-for="item in cityList"
                  :value="item.value"
                  :key="item.value"
                >{{ item.label }}</Option>
              </Select>
            </td>
            <td>
              <Button long @click="transportCapacity_query">查询</Button>
            </td>
          </tr>
          <tr>
            <td>
              <span class="nowtime">当前省份</span>
            </td>
            <td>
              <span class="nowtime">{{transportCapacity_city}}</span>
            </td>
          </tr>
        </table>
        <Table
          :columns="transportCapacity_province_tableHeader"
          :data="transportCapacity_province_tableData"
          :height="tableHeight"
          border
        ></Table>
      </TabPane>
      <TabPane label="按机场统计">
        <table class="transportCapacity_table">
          <tr>
            <td>
              <span class="nowtime">当前时刻</span>
            </td>
            <td>
              <DatePicker
                v-if="false"
                v-model="transportCapacity_airport_time"
                type="datetime"
                placeholder="请选择时间"
                style="width: 200px"
              ></DatePicker>
              <input
                class="DatePicker"
                type="text"
                @click="openByDrop2($event)"
                v-model="calendar4.display"
                readonly
              />
            </td>
            <td>
              <Select v-model="transportCapacity_airport" style="width:200px">
                <Option
                  v-for="item in airportList"
                  :value="item.value"
                  :key="item.value"
                >{{ item.label }}</Option>
              </Select>
            </td>
            <td>
              <Button long @click="transportCapacity_query">查询</Button>
            </td>
          </tr>
          <tr>
            <td>
              <span class="nowtime">当前省份</span>
            </td>
            <td>
              <span class="nowtime">{{transportCapacity_airport}}</span>
            </td>
          </tr>
        </table>
        <Table
          :columns="transportCapacity_airport_tableHeader"
          :data="transportCapacity_airport_tableData"
          :height="tableHeight"
          border
        ></Table>
      </TabPane>
    </Tabs>
    <transition name="fade">
      <div
        class="calendar-dropdown"
        :style="{'left':calendar3.left+'px','top':calendar3.top+'px'}"
        v-if="calendar3.show"
      >
        <calendar
          :zero="calendar3.zero"
          :lunar="calendar3.lunar"
          :value="calendar3.value"
          :begin="calendar3.begin"
          :end="calendar3.end"
          @select="calendar3.select"
        ></calendar>
      </div>
    </transition>
    <transition name="fade">
      <div
        class="calendar-dropdown"
        :style="{'left':calendar4.left+'px','top':calendar4.top+'px'}"
        v-if="calendar4.show"
      >
        <calendar
          :zero="calendar4.zero"
          :lunar="calendar4.lunar"
          :value="calendar4.value"
          :begin="calendar4.begin"
          :end="calendar4.end"
          @select="calendar4.select"
        ></calendar>
      </div>
    </transition>
  </Modal>
</template>
<script>
import dateFormatter from "@/utils/dateFormatter";
import calendar from "@/components/planeMenu/calendar.vue";
export default {
  components: {
    calendar,
  },
  data() {
    return {
      tableHeight: 200,
      transportCapacity_name: 0,
      transportCapacity_show: false,
      transportCapacity_city_time: dateFormatter(
        "yyyy-MM-dd HH:mm:ss",
        new Date()
      ),
      transportCapacity_city: "全国",
      cityList: [
        {
          label: "全国",
          value: "全国",
        },
      ],
      transportCapacity_province_tableHeader: [
        {
          title: "机型",
          key: "type",
          align: "center",
        },
        {
          title: "空中运力",
          key: "air",
          align: "center",
        },
        {
          title: "地面运力",
          key: "ground",
          align: "center",
        },
        {
          title: "合计",
          key: "total",
          align: "center",
        },
      ],
      transportCapacity_province_tableData: [],
      transportCapacity_airport_time: dateFormatter(
        "yyyy-MM-dd HH:mm:ss",
        new Date()
      ),
      transportCapacity_airport: "北京首都",
      airportList: [],
      transportCapacity_airport_tableHeader: [
        {
          title: "机型",
          key: "type",
          align: "center",
        },
        {
          title: "地面运力",
          key: "ground",
          align: "center",
        },
      ],
      transportCapacity_airport_tableData: [],
      calendar3: {
        display: "请选择日期",
        show: false,
        zero: true,
        value: [], //默认日期
        lunar: true, //显示农历
        select: (value) => {
          this.calendar3.show = false;
          this.calendar3.value = value;
          this.calendar3.display = value.join("/");
          this.transportCapacity_city_time = value.join("-") + " 00:00:00";
        },
      },
      calendar4: {
        display: "请选择日期",
        show: false,
        zero: true,
        value: [], //默认日期
        lunar: true, //显示农历
        select: (value) => {
          this.calendar4.show = false;
          this.calendar4.value = value;
          this.calendar4.display = value.join("/");
          this.transportCapacity_airport_time = value.join("-") + " 00:00:00";
        },
      },
    };
  },
  methods: {
    openByDrop(e) {
      this.calendar3.show = true;
      this.calendar3.left = e.target.offsetLeft + 83;
      this.calendar3.top = e.target.offsetTop + 117;

      e.stopPropagation();
      window.setTimeout(() => {
        document.addEventListener(
          "click",
          (e) => {
            this.calendar3.show = false;
            document.removeEventListener("click", () => {}, false);
          },
          false
        );
      }, 1000);
    },
    openByDrop2(e) {
      this.calendar4.show = true;
      this.calendar4.left = e.target.offsetLeft + 83;
      this.calendar4.top = e.target.offsetTop + 117;

      e.stopPropagation();
      window.setTimeout(() => {
        document.addEventListener(
          "click",
          (e) => {
            this.calendar4.show = false;
            document.removeEventListener("click", () => {}, false);
          },
          false
        );
      }, 1000);
    },
    show() {
      console.log(123);
      this.transportCapacity_show = true;
      this.tableHeight = window.innerHeight - 210;
      this.allAirportByBureau();
      this.provinceChinese();
      this.capacityInAirportByProvince();
      this.capacityInAirport();
    },
    close() {
      this.transportCapacity_show = false;
      this.$emit("close", false);
    },
    transportCapacity_query() {
      switch (this.transportCapacity_name) {
        case 0:
          this.capacityInAirportByProvince();
          break;
        case 1:
          this.capacityInAirport();
          break;
        default:
          break;
      }
    },
    transportCapacity_type(name) {
      console.log(name);
      this.transportCapacity_name = name;
    },
    provinceChinese() {
      let url = this.$urlPath.airplane.getProvinceChinese;
      this.$api.get(url).then((res) => {
        if (res.length > 0) {
          this.cityList = [
            {
              label: "全国",
              value: "全国",
            },
          ];
          res.forEach((item) => {
            let temp = {
              label: item,
              value: item,
            };
            this.cityList.push(temp);
          });
          this.transportCapacity_city = "全国";
        }
      });
    },
    //返回全部机场
    allAirportByBureau() {
      let url = this.$urlPath.airplane.getAirportByBureau;
      this.$api.get(url).then((res) => {
        if (res.length > 0) {
          this.airportList = [];
          res.forEach((item) => {
            let temp = {
              label: item,
              value: item,
            };
            this.airportList.push(temp);
          });
          this.transportCapacity_airport = "北京首都";
        }
      });
    },
    capacityInAirportByProvince() {
      let url = this.$urlPath.airplane.getCapacityInAirportByProvince;
      let param = {
        time: this.transportCapacity_city_time,
        province: this.transportCapacity_city,
      };
      this.$api.get(url, param).then((res) => {
        this.transportCapacity_province_tableData = res;
      });
    },
    capacityInAirport() {
      let url = this.$urlPath.airplane.getCapacityInAirport;
      let param = {
        time: dateFormatter(
          "yyyy-MM-dd HH:mm:ss",
          this.transportCapacity_airport_time
        ),
        name: this.transportCapacity_airport,
      };
      this.$api.get(url, param).then((res) => {
        this.transportCapacity_airport_tableData = res;
      });
    },
  },
};
</script>

<style scoped>
.nowtime {
  color: #fff;
}

.DatePicker {
  box-sizing: border-box;
  width: 100%;
  border-radius: 4px;
  border: 1px solid #dedede;
  padding: 6px 10px;
  background: #fff
    url(data:image/svg+xml;base64,)
    no-repeat right 8px center;
  color: #666;
}
/*下拉框*/
.calendar-dropdown {
  background: #fff;
  position: absolute;
  left: 0;
  top: 0;
  padding: 20px;
  border: 1px solid #eee;
  border-radius: 2px;
}
.calendar-dropdown:before {
  position: absolute;
  left: 30px;
  top: -10px;
  content: "";
  border: 5px solid rgba(0, 0, 0, 0);
  border-bottom-color: #dedede;
}
.calendar-dropdown:after {
  position: absolute;
  left: 30px;
  top: -9px;
  content: "";
  border: 5px solid rgba(0, 0, 0, 0);
  border-bottom-color: #fff;
}

.transportCapacity_table {
  color: black;
}
.transportCapacity_table td {
  padding: 5px;
}
span.ivu-date-picker-cells-cell {
  color: black;
}
.screen_box {
  display: flex;
  justify-content: space-between;
  margin-bottom: 15px;
}
.ivu-date-picker-cells span {
  color: black;
}
.ivu-tabs.ivu-tabs-card>.ivu-tabs-bar .ivu-tabs-tab {
   color: black;
}
</style>